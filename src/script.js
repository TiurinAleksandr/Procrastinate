...
button.addEventListener('click', function () {
  let randomElement = getRandomElement(ideas);
  smoothly(idea, 'textContent', randomElement.text);
...
for (let i = 0; i <= 2; i++) {
  smoothly(idea, "textContent", ideas[i].text);
  smoothly(image, "src", ideas[i].image);
}
